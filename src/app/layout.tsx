...
export default function RootLayout({ children }: LayoutProps<"/">) {
  return (
    // ログイン・登録はアプリ内の画面で行う（src/app/sign-in, sign-up）
    <ClerkProvider signInUrl="/sign-in" signUpUrl="/sign-up">
      <html
        lang="ja"
        className={`${geistSans.variable} ${geistMono.variable} ${sawarabiGothic.variable} h-full antialiased`}
      >
        {/* ナビ・本文・フッターを縦に並べ、練習画面はちょうど1画面に収める（globals.css の calc(100dvh - 112px)） */}
        <body className="min-h-full flex flex-col">
          <nav className="studio-nav" aria-label="メインナビゲーション">
            ...
            {/* ログイン中だけ、ページへのリンクとアカウントアイコンを出す（未ログインはログイン画面しか開けないため） */}
            <SignedIn>
              <div className="nav-links"><Link href="/">練習する</Link><Link href="/history">練習の記録 <span aria-hidden="true">↗</span></Link><UserButton /></div>
            </SignedIn>
          </nav>
          {children}
