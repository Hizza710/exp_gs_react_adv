...
export default async function HistoryPage() {
    ...
    const rows = await db.select().from(sessions).where(eq(sessions.userId, userId)).orderBy(desc(sessions.createdAt), desc(sessions.id));
    ...
    return (
        <main className="studio-shell history-dashboard">
            <header className="history-intro"><Link className="muted-copy" href="/">← 練習に戻る</Link><p className="eyebrow" style={{ marginTop: 24 }}>THE PRACTICE COLLECTION</p><h1>積み重ねた、あなたの言葉。</h1><p className="intro-copy">練習の記録 / {rows.length} 件。一回ごとの気づきが、次の自信につながります。</p></header>
