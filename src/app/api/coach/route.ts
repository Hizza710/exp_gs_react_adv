...
export async function POST(request: Request) {
  ...
  // 受け取るフィールドを拡張：topic, tone, mode, answers, coachQuestion, smileScore
  const { topic, answer, tone, mode, answers, coachQuestion, smileScore } = body;
  ...
  if (coachQuestion === "consistency") {
    ...
    prompt = `あなたはプレゼン/面接の練習コーチです。\n「${t}」な口調で、次の「お題」に対する「回答」を読んで、良かった点と改善点を、具体的に、200文字くらいで日本語でフィードバックしてください。${smileNote}\nお題: ${top}\n回答: ${safe(answer)}`;
  }
